...
class ClusteringSolver:
    ...
    def plot_elbow_method(self, wcss, silhouette_scores, optimal_k, max_k=8):
        """
        Построение графика метода локтя с оценками силуэта
        """
        ...
        ax1.axvline(x=optimal_k, color='red', linestyle='--', alpha=0.7, label=f'Оптимальное k = {optimal_k}')
        ...
        # График оценок силуэта
        ax2.plot(k_values, silhouette_scores, 'go-', linewidth=2, markersize=8)
        ax2.axvline(x=optimal_k, color='red', linestyle='--', alpha=0.7, label=f'Оптимальное k = {optimal_k}')
        ax2.set_xlabel('Количество кластеров (k)')
        ax2.set_ylabel('Silhouette Score')
        ax2.set_title('Оценки силуэта для разных значений k')
        ...
        print(f"WCSS при k={optimal_k}: {wcss[optimal_k - 2]:.2f}")
        print(f"Silhouette Score при k={optimal_k}: {silhouette_scores[optimal_k - 2]:.3f}")

        # Интерпретация силуэта
        silhouette_value = silhouette_scores[optimal_k - 2]
        if silhouette_value >= 0.7:
            print("Отличное качество кластеризации (сильная структура)")
        elif silhouette_value >= 0.5:
            print("Хорошее качество кластеризации (разумная структура)")
        elif silhouette_value >= 0.25:
            print("Удовлетворительное качество кластеризации (слабая структура)")
        else:
            print("Плохое качество кластеризации (отсутствие значимой структуры)")
    ...
    def perform_clustering(self, df, n_clusters=None):
        ...
        if n_clusters is None:
            ...
            self.plot_elbow_method(wcss, silhouette_scores, n_clusters)
        else:
...
test_data = pd.read_csv('E:\PY\MatFlow-Analytics\csvFiles\Task2Csv.csv', sep=';', encoding='utf-8')

# Выполняем кластеризацию с автоматическим определением количества кластеров
result_df, cluster_stats, metrics = clustering_solver.perform_clustering(test_data)

# Выводим результаты
print("\nСТАТИСТИКА КЛАСТЕРОВ:")
print(cluster_stats)

print("\nРАСПРЕДЕЛЕНИЕ ПОСТАВЩИКОВ ПО КЛАСТЕРАМ:")
print(result_df[['Название поставщика', 'cluster']].sort_values('cluster'))